...
namespace UnityStandardAssets.Water
{
    ...
    public class PlanarReflection : MonoBehaviour
    {
        ...
        public void Start()
        {
            ...
            WaterBase.AddWaterComponent(this);
            ApplyFromConfig();
        }
        ...
        public void RenderHelpCameras(Camera currentCam)
        {
            if(m_isDeleteCamera )
            {
                if (_deleteCameraCount > 0)
                {
                    _deleteCameraCount--;
                    String reflName = "/" + gameObject.name + "Reflection" + currentCam.name;
                    GameObject go = GameObject.Find(reflName);
                    if (go != null)
                    {
        ...
        bool _isDeleteCamera;
        int _deleteCameraCount;
        bool m_isDeleteCamera
        {
            get { return _isDeleteCamera; }
            set
            {
                _deleteCameraCount = 10;
                _isDeleteCamera = value;
            }
        }
        private void OnDestroy()
        {
            WaterBase.RemoveWaterComponent(this);
        }
        //��ȡ����
        public void ApplyFromConfig()
        {
            if (WaterBase.m_cfgWaterQuality == WaterBase.WaterQualityUser.high
                || WaterBase.m_cfgWaterQuality == WaterBase.WaterQualityUser.veryHigh
                || WaterBase.m_cfgWaterQuality == WaterBase.WaterQualityUser.medium)
            {//��������
                m_isDeleteCamera = false;
                //edgeBlend = false;
                if (WaterBase.m_cfgWaterQuality == WaterBase.WaterQualityUser.medium)
                {
                    //�������
                    reflectionMask = 0;
                }
                else if (WaterBase.m_cfgWaterQuality == WaterBase.WaterQualityUser.high)
                {
                    //�������,����,����Զ��ữ��Ե
                    reflectionMask = 1<<LayerMask.NameToLayer("terrain");
                }
                else if(WaterBase.m_cfgWaterQuality == WaterBase.WaterQualityUser.veryHigh)
                {//����ȫ��,����Զ��ữ��Ե
                    uint mask = 0xffffffff;
                    reflectionMask = (LayerMask)(mask & ~(
