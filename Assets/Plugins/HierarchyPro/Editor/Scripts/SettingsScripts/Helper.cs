	...
	class Draw
	{
		...
		internal static void BACK_BUTTON( IRepaint w )
		{
			...
			if ( w is MainRoot )
			{
				...
				if ( Draw.BUT_S( r, "<--  Back To Settings List", _b ) ) { MainSettingsEnabler_Window.Select<MainSettingsEnabler_Window>( skipOpenWindow: true ); }
				GUI.color = C;
		...
		internal static bool TOG_TIT( string text, string KEY, Func<FIELD_SETTER, bool> valudate = null, Action<FIELD_SETTER> onChanged = null, float rightOffset = 0, Rect? rov = null, object overrideObject = null, string AreYouSureText = null, bool? EnableRed = null )
		{
		...
		internal static void TOG_TIT( string text, float rightOffset = 0, bool? EnableRed = null )
		{
			TOG_TIT( CONT( text ), rightOffset, EnableRed );
		}
		internal static void TOG_TIT( GUIContent text, float rightOffset = 0, bool? EnableRed = null )
		{
			//   var setter = GetSetter( KEY );
			var oldP = Draw.padding;
			Draw.padding = 0;
		...
		{
			var setter = valudate == null ? Draw.GetSetter(KEY, overrideObject: overrideObject) : Draw.GetSetter(KEY, onChanged, valudate: valudate, overrideObject: overrideObject);
			var res = EditorGUI.ToggleLeft(rov ?? R, (text), setter.AS_BOOL);
		...
		internal static void TOG( string text, string KEY, Func<FIELD_SETTER, bool> valudate = null, Action<FIELD_SETTER> onChanged = null, Rect? rov = null, object overrideObject = null, bool EnableRed = false )
		{
			...
			//if ( Draw.padding > 20 )
			{
				var res = EditorGUI.ToggleLeft(rov ?? R, CONT(text), setter.AS_BOOL);
				if ( res != setter.AS_BOOL ) setter.AS_BOOL = res;
		...
		static GUIStyle tb;
		internal static void TOOLBAR( string[] textArray, string KEY, float height = -1, object overrideObject = null, Rect? rect = null, string[] tooltips = null, bool[] enabled = null )
		{
			...
			for ( int i = 0; i < textArray.Length; i++ )
			{
				...
				if ( GUI.enabled )
				{
					...
					if ( Event.current.type == EventType.Repaint && GUI.enabled ) tb.Draw( r, cont, controlId, i == b, r.Contains( Event.current.mousePosition ) );
					Root.SetMouseTooltip( cont, r );
		...
		internal static string FOLDER( IRepaint w, string text, string KEY, AboutCacheEditor.CacheFolderDrawer.FOLDER_DATA d )
		{
		...
		internal static string FOLDER( IRepaint w, string text, string KEY,
			AboutCacheEditor.CacheFolderDrawer.FOLDER_DATA d, out Action<string, string[], string[]> outFinalization, bool drawDecorations = true, string cf_String = null )
		{
			...
			using ( drawDecorations ? MainRoot.GRO( w ).UP( 20 ) : new CLASS_GROUP.dsp() { skip = true } )
			{
				...
				if ( drawDecorations )
				{
					...
					{
						MainSettingsEditor.REV( d.prefix + v );
					}
					_R2.x += _R2.width;
				}
...
findl:;
		...
		internal static int S_INT_FIELD( Rect rect, int value, string postFix = null, float lablelOffset = 0 )
		{
			...
			value = EditorGUI.IntField( rect, "ASD", value );
			GUI.EndClip();
			GUI.color = ac;
