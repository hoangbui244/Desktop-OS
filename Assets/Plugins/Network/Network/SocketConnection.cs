...
namespace NGE.Network
{
	...
	public abstract class SocketConnection : /*UnManagedResource,*/ IConnection
	{
		...
		internal SocketConnection(Socket socket, bool secure)
		{
			...
			m_RecvBuffer = new byte[BUFFER_SIZE + 8192/*8k,why?*/];
		...
		public int Connect(IPEndPoint endPoint, bool secure, object state)
		{
			...
			if (m_socket == null)
				CreateSocket();
			m_socket.BeginConnect(endPoint, m_Connected, null);
		...
		public void Send(byte[] data, int count)
		{
			...
			try
			{
				...
				//
				if (sendnow)
					m_socket.BeginSend(data, 0, count, SocketFlags.None, m_OnSent, null);
			}
		...
		private void OnEndSend(IAsyncResult rs)
		{
			...
			{
				//socket be closed .. 
				CatchConnectionLost(ErrorType.SendFailed, "connection closed.");
			}
			catch (ObjectDisposedException)
			{
				CloseConnection();
...
#endif
					m_socket.BeginReceive(data, offset, count, SocketFlags.None, m_OnReceived, null);
				}
			}
			catch (SocketException)
			{
