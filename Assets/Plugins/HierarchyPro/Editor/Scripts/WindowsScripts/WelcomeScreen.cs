...
namespace EMX.HierarchyPlugin.Editor
{
	...
	class WelcomeScreen : EditorWindow, Icons.GetTextureObject, IRepaint
	{
		...
		string[] Chapters = { "HELLO!" , "CACHE FOLDERS",  "INTRO", 
#if !TRUE
            "HIGHLIGHTER",
#endif
		...
		private void OnGUI()
		{
			...
			for ( int i = 0; i < Chapters.Length; i++ )
			{
				...
				{
					GUILayout.Space( 16 );
					GUILayout.Label( "Help Categories:" );
					GUILayout.Space( 6 );
				}
			}
...
""Welcome Screen"" includes ""HELP"" pics and description of different parts of "+ Root.HierarchyPro+"." );
				GUILayout.Space( 20 );
...
and you can send your 'Scene' to those who haven't the " + Root.HierarchyPro +"." );
				...
				Draw.HRx05( EditorGUILayout.GetControlRect() );
				GUILayout.Space( 20 );
				GUILayout.Label( @"Try these quick presets, if you are new:" );
				...
				GUILayout.Label( @"You can also try to use quick toggles at the top, all these settings you can find in the hierarchy settings." );
			...
			else if ( currentWelcomeChapter == 1 )
			{
				//
				//GUILayout.Space( 20 );
				GUI.Label( Draw.R, "Yay, you can change cache folder now, or in the settings later:" );
				...
				AboutCacheEditor.DRAW_CACHE_BUTTONS_WITH_ENABLER( this );
			}
			else
			{
			...
			{
				Settings.MainSettingsEnabler_Window.Select<Settings.MainSettingsEnabler_Window>( true );
			}
			Draw.HRx1( EditorGUILayout.GetControlRect() );
