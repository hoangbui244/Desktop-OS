    ...
    */
	...
	internal class Mod_UserModulesRoot : RightModBaseClass
	{
		...
		public Mod_UserModulesRoot( int restWidth, int sibbildPos, bool enable, PluginInstance adapter ) : base( restWidth, sibbildPos, enable, adapter )
		{
			...
			CustomHierarchyMod.m_ChangeCursorAccordingWithSettings = ChangeCursorAccoringWithSettings;
			CustomHierarchyMod.m_BUTTON_AccordingWithSettings = BUTTON_AccordingWithSettings;
			CustomHierarchyMod.m_BUTTON_AccordingWithSettings2 = BUTTON_AccordingWithSettings2;
		}
		...
		bool BUTTON_AccordingWithSettings( Rect rect, string content, bool hasContent )
		{
			...
			var result= adapter.ModuleButton(rect,emptyContent, hasContent, EMX.Utility.Styles.button, out allowClick);
			if ( allowClick ) ChangeCursorAccoringWithSettings( rect );
			return result;
		...
		{
			bool allowClick;
			var result= adapter.ModuleButton(rect,content, hasContent, style ?? EMX.Utility.Styles.button, out allowClick);
			if ( allowClick ) ChangeCursorAccoringWithSettings( rect );
			return result;
