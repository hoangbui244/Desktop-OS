	...
	public virtual void Show ()
	{
		if (enabled && NGUITools.GetActive(gameObject) && mChild == null && isValid && items.Count > 0)
		{
			...
			if (separatePanel)
			{
                ...
                {
                    UIPanel tempPanel = mChild.AddComponent<UIPanel>();
                    tempPanel.depth = 1000000;
                    string name = "ui4";//非editor模式下暂时写死  [luozhijia 2018-05-07]
#if UNITY_EDITOR
...
#endif
                    tempPanel.sortingLayerName = name;
                }
            }
            current = this;
