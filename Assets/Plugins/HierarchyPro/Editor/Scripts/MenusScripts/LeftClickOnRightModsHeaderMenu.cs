...
namespace EMX.HierarchyPlugin.Editor
{
	class LeftClickOnRightModsHeaderMenu
	{
		...
		internal static void Open( bool addMenu = false, Action<GenericMenu> topBuilder = null )
		{
...
#if !TRUE
			var ws = p.par_e;
			menu.AddItem( new GUIContent( "[ Use bottom bar mod ]" ), ws.USE_BOTTOMBAR_MOD, () => { ws.USE_BOTTOMBAR_MOD = !ws.USE_BOTTOMBAR_MOD; } );
#endif

			ADD_LAYOUTS( ref menu );
			menu.AddSeparator( "" );

			menu.AddItem( new GUIContent( "[ Open settings - right mods bar ]" ), false, () => {
			...
			if ( s.IsValid() && !string.IsNullOrEmpty( s.path ) ) menu.AddItem( new GUIContent( "> Select scene file in project" ), false, () => {
				UnityEditor.EditorUtility.FocusProjectWindow();
				var ap = AssetDatabase.LoadAssetAtPath<SceneAsset>( s.path );
				Selection.objects = new[] { ap };
			} );
			else menu.AddDisabledItem( new GUIContent( "> Select scene file in project" ) );
			menu.AddItem( new GUIContent( "> Select cache file in project" ), false, () => {
		...
		static void CONTEXTMENU_STATICMODULES( GenericMenu menu )
		{
			...
			cont = new GUIContent( "- " + p.modsController.playModeKeeperMod.ContextHelper.ToString() + " -" );
			menu.AddItem( cont, p.par_e.USE_PLAYMODE_SAVER_MOD, () => {
				p.par_e.USE_PLAYMODE_SAVER_MOD = !p.par_e.USE_PLAYMODE_SAVER_MOD;
			} );
#endif

			cont = new GUIContent( "- " + p.modsController.componentsIconsMod.SearchHelper.ToString() + " -" );
			...
			cont = new GUIContent( "- " + p.modsController.setActiveMod.ContextHelper.ToString() + " -" );
		...
		static void ADD_LAYOUTS( ref GenericMenu menu )
		{
			...
			const string CATEGORY = "[ Right mods layouts ]/";
