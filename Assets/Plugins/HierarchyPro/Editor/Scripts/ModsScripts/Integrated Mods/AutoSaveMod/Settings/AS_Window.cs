...
using static EMX.HierarchyPlugin.Editor.Settings.AboutCacheEditor;
...
namespace EMX.HierarchyPlugin.Editor.Settings
{
	...
	class AutosaveSceneModSettingsEditor : MainRoot
	{
		...
		internal static string set_text =  USE_STR + "Autosave Scene Every n'min Mod (Background)";
		...
		public static void _GUI( IRepaint w )
		{
			...
			using ( ENABLE( w ).USE( set_key ) )
			{
				...
				using ( GRO( w ).UP( 0 ) )
				{
					...
					using ( new fd( () => Draw.FOLDER_PAD = 20, () => Draw.FOLDER_PAD = 0 ) ) Draw.FOLDER( w, "Auto-Save Location:", "AS_LOCATION", data );
...
hh mm ss - time" );
					...
					Draw.HELP( w, "Autosave pauses the time during the PlayMode, for example, you set save every 5 minutes, and 2 minutes left to save, you will play 15 minutes and when you stop, 2 minutes will also be left to save" );
					Draw.Sp( 10 );
