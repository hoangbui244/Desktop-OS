...
namespace EMX.HierarchyPlugin.Editor.Mods
{
	...
	partial class RightModsManager
	{
		...
		internal static string GetEmptyConten = "";
		...
		void PreCalcRect()
		{
			...
			baked_SHRINK_BUTTONS = p.par_e.RIGHTDOCK_SHRINK_BUTTONS;
			baked_CHANGE_CURSOR = p.par_e.RIGHTDOCK_CHACGE_CURSOR;
			if ( p.par_e.RIGHT_DRAW_HYPHEN_FOR_EMPTY_LABELS && GetEmptyConten != "-" ) GetEmptyConten = "-";
			if ( !p.par_e.RIGHT_DRAW_HYPHEN_FOR_EMPTY_LABELS && GetEmptyConten != "" ) GetEmptyConten = "";
		...
		void Draw_Last()
		{
			...
			if (p.par_e.RIGHT_HEADER_BG_OPACITY != 0 )
			{
				...
				EditorGUI.DrawRect( headerEventsBlockRect.Value, casd );
			}
