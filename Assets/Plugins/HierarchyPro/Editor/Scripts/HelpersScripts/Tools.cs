    ...
    partial class Tools
    {
                ...
                string extension = null )        // in this example we return less than N if necessary
        {
            ...
            {   // var paths =  System.IO.Directory.GetFiles( Application.dataPath,"*.*",System.IO.SearchOption.AllDirectories ).Where(p=>!p.EndsWith(".meta")).Select(p=>p.Replace('\\','/').Substring(LLL)).ToArray();
                ...
                {
                    if ( !paths[ i ].StartsWith( "Assets", StringComparison.OrdinalIgnoreCase ) ) continue;
                    if ( haveActiveParentTransform && !paths[ i ].StartsWith( activePath , StringComparison.OrdinalIgnoreCase) ) continue;
                    if ( extension != null && !paths[ i ].EndsWith( extension , StringComparison.OrdinalIgnoreCase) ) yield return null;
        ...
        {
            _SCAN_FIELDS_CACHE = new Dictionary<Type, ConcurrentDictionary<string, FieldAdapter>>();
            _SCAN_FIELDS_CACHE_NOARRAYS = new Dictionary<Type, ConcurrentDictionary<string, FieldAdapter>>();
        }
        ...
        internal static ConcurrentDictionary<string, FieldAdapter> GET_FIELDS( Type type, bool includeArrays = true )
        {
            ...
            {
                lock ( SCAN_FIELDS_CACHE ) SCAN_FIELDS_CACHE.Add( type, new ConcurrentDictionary<string, FieldAdapter>() );
