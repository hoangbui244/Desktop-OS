...
namespace EMX.HierarchyPlugin.Editor
{
	...
	public class Root
	{
		...
		internal static void SetMouseTooltip( GUIContent content, Rect rect )
		{
			...
			if ( !rect.Contains( Root.p[ 0 ].EVENT.mousePosition ) || !Root.p[ 0 ].trueNulled_GUIClip_visibleRect.Contains( Root.p[ 0 ].EVENT.mousePosition ) ) return;
...
#pragma warning restore
		...
		internal class AdapterData
		{
			...
			internal MousePos GET_CURRENT_MOUSE_POS_FOR_MENU {
				get {
					...
					if ( Mathf.Abs ( bakedRightClickFrame - fullFrame ) > 5) return new MousePos( Event.current.mousePosition, MousePos.Type.Input_190_68, true, Root.p[0] );
					return bakedRightClickPos ?? new MousePos( Event.current.mousePosition, MousePos.Type.Input_190_68, true, Root.p[0] );
				}
			}
		}
		...
		internal void PUSH_GUI_ONESHOT_FORCE_FIRSTONLY( int plug, Action ac )
		{
			bool allow = false;
			foreach ( var w in WindowsData( plug ) ) if ( w.Value.w.Instance ) allow = true;
			if ( !allow )
 ...
 internal IList<int> current_DragSelection_List = new List<int>();
		...
		void CHECK_ONESHOT_GUI()
		{
			...
			{
				foreach ( var item in _oneShotGuiForceFirstOnly[ pluginID ] ) item();
		...
		internal void gui( int instanceID, ref Rect selectionRect )
		{
			...
			if ( GuiReady/* || !_go*/  || lastEvent[ pluginID ] != EVENT.type )
			{
				...
				if ( lastWindow[ pluginID ] && lastWindow[ pluginID ] != window.Instance ) return;
