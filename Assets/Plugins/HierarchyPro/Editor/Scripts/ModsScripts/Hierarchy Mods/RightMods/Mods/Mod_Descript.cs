    ...
    internal class Mod_Descript : RightModBaseClass, IModSaver
    {
        ...
        public override void Draw()
        {
            ...
            if ( data != null && !string.IsNullOrEmpty( data.stringValue ) )
            {
                ...
                Draw_Label( drawRect, content, style, true );
                hasContent = true;
            ...
            else
            {
                if ( adapter.par_e.RIGHT_SKIP_HYPHEN_FOR_DESCRIPTIONS )
                {
                    ...
                    r = adapter.modsController.rightModsManager.DrawCursorRect( ref drawRect, style, "" );
                }
                else
                {
                    if ( content.text != RightModsManager.GetEmptyConten ) content.text = RightModsManager.GetEmptyConten;
                    if ( !string.IsNullOrEmpty( content.text ) ) Draw_Label( drawRect, content, style, true );
                    r = adapter.modsController.rightModsManager.DrawCursorRect( ref drawRect, style, RightModsManager.GetEmptyConten );
                }
            }
