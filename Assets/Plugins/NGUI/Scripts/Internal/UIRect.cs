...
public abstract class UIRect : MonoBehaviour
{
    ...
    //当更新模式设置为Update时,是否已经更新过
    bool m_updateAnchorsWhileUpdate;
	...
	protected virtual void OnEnable ()
	{
		...
		if (updateAnchors == AnchorUpdate.OnEnable|| updateAnchors == AnchorUpdate.OnUpdate)
		{
			mAnchorsCached = false;
			mUpdateAnchors = true;
            m_updateAnchorsWhileUpdate = false;
        }
		if (mStarted) OnInit();
		mUpdateFrame = -1;
	}
...
#endif
            {
                //if(!m_updateAnchorsWhileUpdate)
                {
                    //m_updateAnchorsWhileUpdate = true;
