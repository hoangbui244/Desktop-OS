...
public class UIGrid : UIWidgetContainer
{
    ...
    //是否使用新的的居右的属性
    public bool isUseNewTopRight = false;
	/// <summary>
	...
	protected virtual void ResetPosition (List<Transform> list)
	{
		...
		if (pivot != UIWidget.Pivot.TopLeft)
		{
            ...
            for (int i = 0; i < myTrans.childCount; ++i)
			{
                ...
                //新增居右类型　by guopengjie
                if (isUseNewTopRight && pivot == UIWidget.Pivot.TopRight && arrangement == Arrangement.Horizontal)
                {
                    if (sp != null)
                    {
                        sp.target.x = -sp.target.x;
                        sp.target.y -= fy;
                    }
                    else
                    {
