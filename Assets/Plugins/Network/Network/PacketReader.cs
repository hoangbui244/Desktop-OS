...
namespace NGE.Network
{
	...
	public sealed class PacketReader
	{
        ...
        private int m_offset_init;
        private int m_Size_init;
        ...
        public bool MoveToNextPacket(bool isFirstMove)
        {
            short curPacketLength = 0;
            if (isFirstMove)
            {
                curPacketLength = (short)Packet.HeaderSize;
            }
            else
            {
                curPacketLength = Util.ArrayUtility.GetShort(m_Data, m_offset + Packet.OffsetLength);
            }
            //如果不够下一个消息包头大小，则表示
            if (m_offset+ curPacketLength+Packet.HeaderSize < m_Size_init)
            {
                m_offset += curPacketLength;
                m_Index = m_offset + Packet.HeaderSize;
                return true;
            }
            return false;
		...
		/// </summary>
		/// <returns></returns>
		public string ReadUTF8WithBufferLength(int bufferLength)
		{
			...
			int count = 0;
			int index = m_Index;
			while (index < m_Size + m_offset && m_Data[index++] != 0&&count<= bufferLength)
				++count;
			index = 0;
            if (count>0)
            {
                s = Utility.UTF8.GetString(m_Data, m_Index, count);   
            }
            m_Index += bufferLength;
            return s;
        }
        public string ReadStringAsUTF8()
        {
            if (m_Index >= m_Size + m_offset)
                return String.Empty;
            int count = 0;
            ...
            while (index < m_Size + m_offset && m_Data[index++] != 0)
                ++count;
            index = 0;
            string s = Utility.UTF8.GetString(m_Data, m_Index, count);
            m_Index += count + 2;
            bool isSafe = true;
        ...
        public string ReadUTF8FromCPPServer()
        {
            int size = ReadCompressInt();
            string s = Utility.UTF8.GetString(m_Data,m_Index,size);
            m_Index += size;
            return s;
        }
        /// <summary>
        ...
        public string ReadUTF8(int fixedLength)
        {
            ...
            string s = Utility.UTF8.GetString(m_Data, m_Index, fixedLength);
            m_Index += fixedLength;
            return s;
        }
        /// <summary>
        /// 以确定长度读取一个utf8编码的字符串,并且保证读取指定的长度
        /// </summary>
        /// <param name="fixedLength"></param>
        /// <returns></returns>
        public string ReadUTF8SkipFixedBufferLength(int stringLength,int bufferLength)
        {
            if (stringLength <= 0)
            {
                m_Index += bufferLength;
            ...
            string s = Utility.UTF8.GetString(m_Data, m_Index, stringLength);
            m_Index += bufferLength;
            return s;
        }
        /// <summary>
